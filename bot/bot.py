...
async def startup():
	if self.configuration.read(category="utilities", key="developer.extensions.load"): # - If load extensions (cogs)-
		...
		for cog in os.listdir(self.configuration.read(category="utilities", key="developer.extensions.directory")):
			if cog.endswith('.py'):	 # - Every file from directory path with .py extension is handled as a potential cog. -
				if not cog[:-3] in self.configuration.read(category="utilities", key="developer.extensions.ignore"): # - If cog is not ignored ( in ignored list ) -
